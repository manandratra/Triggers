...
Runner.controls.MapManager = Runner.extend( Runner.emptyFn, {
	 ...
	 * @return {object}
	 */
	getMarkerDataByRecId: function( mapId, recId ) {
		var mapData = this.mapsData[ mapId ], i;
		
		for ( var i = 0; i < ( mapData.markers || [] ).length; i++ ) {
			if ( mapData.markers[ i ].recId === recId ) {
				return mapData.markers[ i ];
			}
		}
	 ...
	 */
	_reinitMapMarkers: function( mapId, newMarkersData, updateCenter, destroyOldMarkers ) {
		var mapData = this.mapsData[ mapId ],
			activeMarkerData;
		...
		if (  mapData.dashMap && !this.isHeatMap( mapData ) ) {
			activeMarkerData = activeMarkerData && this.getMarkerDataByKeys( mapId, activeMarkerData.keys ) || mapData.markers[0];
