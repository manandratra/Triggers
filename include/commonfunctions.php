...
function checkTableName($shortTName, $type=false)
{
	...
	if ("audit_vendeurs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("recettes_jour" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("recettes_mois" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("recettes_vendeur_mois" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("recettes_vendeurs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("vendeurs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Dashboard" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("recettes_vendeur_mois_Chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	if ("admin_rights" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("vendeurs_triggers__users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
{
	$arr = array();
	$strPerm = GetUserPermissions("audit_vendeurs");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="audit_vendeurs";
	}
	$strPerm = GetUserPermissions("recettes_jour");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="recettes_jour";
	}
	$strPerm = GetUserPermissions("recettes_mois");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="recettes_mois";
	}
	$strPerm = GetUserPermissions("recettes_vendeur_mois");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="recettes_vendeur_mois";
	}
	$strPerm = GetUserPermissions("recettes_vendeurs");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="recettes_vendeurs";
	}
	$strPerm = GetUserPermissions("vendeurs");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="vendeurs";
	}
	...
	$strPerm = GetUserPermissions("recettes_vendeur_mois Chart");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="recettes_vendeur_mois Chart";
	}
	...
	$strPerm = GetUserPermissions("vendeurs(triggers)_users");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="vendeurs(triggers)_users";
	}
...
{
	if($shorttable=="recettes_vendeur_mois_Chart")
		return "2DPie";
	return "";
}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		if(!$caseInsensitiveUsername)
		{
			$sql = "select ".$grConnection->addFieldWrappers( "GroupID" )
				...
				." from ". $grConnection->addTableWrappers( "vendeurs(triggers)_ugmembers" )
	...
	$sql = "select ". $rConnection->addFieldWrappers( "TableName" )
		...
		." from ". $rConnection->addTableWrappers( "vendeurs(triggers)_ugrights" )
...
function guestHasPermissions()
{
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("audit_vendeurs",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("recettes_jour",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("recettes_mois",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("recettes_vendeur_mois",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("recettes_vendeurs",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("vendeurs",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("Dashboard",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("recettes_vendeur_mois Chart",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("admin_rights",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("vendeurs(triggers)_users",$_SESSION["UserRights"]["Guest"]))
		return true;
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("vendeurs(triggers)_users")
