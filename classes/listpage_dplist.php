...
//			array( "name" => "record_controls_block", "align" => "left" ) 
	...
	protected function getHeaderControlsBlocks()
	{
			...
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
			{
				$controlsBlocks = '<span class="rnr-dbebrick ">'
					.'<div class="style1 rnr-bl rnr-b-recordcontrols_new">'
						.'<a class="rnr-button" href="#" '.$inlineaddlink_attrs.'>'."Ajouter".'</a> '
					.'</div>'
				.'</span>';		
			}
			else
			{
				$buttons .= '<a class="' . $bs_button_class . '" href="#" '.$inlineaddlink_attrs.'>'."Ajouter".'</a> ';		
			}
		}
		...
		if( $this->inlineEditAvailable() && $this->xt->getVar("editselected_link") )
		{
			...
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
				$buttons.= '<a class="rnr-button bs-invisible-button" href="#" '.$editselectedlink_attrs.' '.$editselectedlink_span.'>'."Editer".'</a> ';
			else
				$buttons.= '<a class="' . $bs_button_class . ' bs-invisible-button" href="#" '.$editselectedlink_attrs.' '.$editselectedlink_span.'>'."Editer".'</a> ';
		};	
		...
		if( $this->deleteAvailable() && $this->xt->getVar("deleteselected_link") )
		{
			...
			// "bs-invisible-button" class need for init hidden in bootstrap
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
				$buttons.= '<a class="rnr-button bs-invisible-button" href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'."Supprimer".'</a> ';		  
			else
				$buttons.= '<a class="' . $bs_button_class . ' bs-invisible-button" href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'."Supprimer".'</a> ';		  
		}		
