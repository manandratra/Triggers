...
class LookupField extends EditControl
{
	...
	{		
		...
		if($this->bUseCategory)
		{
			...
			{			
				...
				echo '&nbsp;<a href="#" id="'.$this->openlookup.'"'.$visibility.'>'."Sélectionner".'</a>';
			}				
			
			echo '<input type="hidden" id="'.$this->cfield.'" '.$valueAttr.' name="'.$this->cfield.'"'.$dataAttr.'>';
			
			if( $this->addnewitem )
				echo "&nbsp;<a href=# id='addnew_".$this->cfield."'>"."Ajouter"."</a>";
	...
	protected function buildMultiselectAJAXLookup($avalue, $value, $mode, $searchOption)
	{
		...
		if( $this->LCType == LCT_LIST )
			echo $forLinkView.'<a href="#" id="'.$this->openlookup.'">'."Sélectionner".'</a>';
		
		if( $this->addnewitem )
			echo $forLinkView."<a href=# id='addnew_".$this->cfield."'>"."Ajouter"."</a>";				
	}
